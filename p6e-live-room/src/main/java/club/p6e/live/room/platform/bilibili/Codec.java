...
public class Codec extends LiveRoomCodec<Message> {
     ...
     * @return 转换后的消息对象集合
     */
    private List<club.p6e.live.room.platform.douyu.Message> decodeByteBufToMessages(ByteBuf byteBuf) {
        // 消息对象的集合
        final List<club.p6e.live.room.platform.douyu.Message> result = new ArrayList<>();
        try {
                ...
                if (byteBuf.readableBytes() > CONTENT_LENGTH_HEADER_BYTE_LENGTH) {
                    ...
                    if (len1 > HEADER_LENGTH && byteBuf.readableBytes() >= len1) {
                        ...
                        if (len1 == len2 && type == ROOM_RECEIVE_MESSAGE_TYPE) {
                            final club.p6e.live.room.platform.douyu.Message message = this.builder.deserialization(contentBytes);
